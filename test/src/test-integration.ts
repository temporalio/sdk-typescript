...
import { Connection, compileWorkflowOptions, addDefaults } from '../../lib/workflow-client';
...
test('WorkflowOptions are passed correctly with defaults', async (t) => {
  ...
  t.deepEqual(
    execution.workflowExecutionInfo?.memo,
    iface.temporal.api.common.v1.Memo.create({ fields: {} }),
  );
  t.deepEqual(
  ...
  t.is(
    execution.executionConfig?.taskQueue?.kind,
...
test('WorkflowOptions are passed correctly', async (t) => {
  const client = new Connection();
  const opts = compileWorkflowOptions(addDefaults({
    taskQueue: 'test2',
    memo: { a: 'b' },
  ...
  t.deepEqual(
    defaultDataConverter.fromPayload(execution.workflowExecutionInfo?.memo?.fields?.a!),
    'b',
  );
  t.deepEqual(
