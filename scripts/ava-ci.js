...
function findWorkspaceRoot(start) {
    ...
    for (;;) {
        ...
        if (parent === dir)
            return start; // fall back to cwd if not found
        dir = parent;
    }
}
const root = findWorkspaceRoot(cwd);
let pkgName = (0, node_path_1.basename)(cwd);
try {
    const pkg = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.join)(cwd, 'package.json'), 'utf8'));
    if (pkg.name)
        pkgName = pkg.name;
}
catch {
...
const resultsDir = process.env.TEST_RESULTS_DIR || (0, node_path_1.join)(root, '.test-results');
(0, node_fs_1.mkdirSync)(resultsDir, { recursive: true });
const logPath = (0, node_path_1.join)(resultsDir, `${safeName}.log`);
const jsonPath = (0, node_path_1.join)(resultsDir, `${safeName}.json`);
const logStream = (0, node_fs_1.createWriteStream)(logPath);
...
let pendingFailure = null; // set while consuming a diagnostic block
...
function handleTapLine(rawLine) {
...
const [cmd, cmdArgs] = process.env.AVA_RUNTIME === 'bun'
    ? ['bun', ['run', '-b', 'ava', '--tap', ...forwarded]]
    : [(0, node_path_1.join)(cwd, 'node_modules', '.bin', process.platform === 'win32' ? 'ava.cmd' : 'ava'), ['--tap', ...forwarded]];
...
function finish(exitCode) {
    ...
    logStream.end();
    try {
        (0, node_fs_1.writeFileSync)(jsonPath, JSON.stringify(result, null, 2));
    }
    catch {
        // non-fatal: summary aggregation just won't include this package
    }
    const relLog = (0, node_path_1.relative)(root, logPath);
    const parts = [`${pass} passed`];
    if (skip)
        parts.push(`${skip} skipped`);
    if (todo)
        parts.push(`${todo} todo`);
    if (noTestFiles.length)
        parts.push(`${noTestFiles.length} file(s) without tests`);
    ...
    if (succeeded) {
        ...
        if (fail === 0) {
            const tail = (0, node_fs_1.readFileSync)(logPath, 'utf8').split('\n').slice(-40).join('\n');
