...
function emit(): string {
  ...
  // System Nexus envelopes are reconstructed as protobufjs messages in the Worker.
  // Dispatch from their descriptor identity, just as the Python SDK's generic
  // PayloadVisitor does, rather than requiring callers to name a generated walker.
  const genericEntry = [
    `export function walkPayloadsInMessage<Ctx>(`,
    `  root: object,`,
    `  env: WalkEnv<Ctx>,`,
    `  context: Ctx`,
    `): Promise<unknown>[] {`,
