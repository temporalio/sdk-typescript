...
// Arc because most of CoreWorker's API require sending reference to the Worker into async closures.
type CoreWorker = Arc<temporal_sdk_core::Worker>;
...
pub fn worker_new(
    ...
    let client_ref = client.borrow_inner()?;
...
pub fn worker_validate(worker: OpaqueInboundHandle<Worker>) -> BridgeResult<BridgeFuture<()>> {
    let (runtime, worker) = worker.map_inner(|worker| {
        (
            Arc::clone(&worker.core_runtime),
            Arc::clone(&worker.core_worker),
        )
    })?;
...
pub fn worker_record_activity_heartbeat(
    ...
    let worker_ref = worker.borrow_inner()?;
    worker_ref
        .core_worker
...
pub fn worker_finalize_shutdown(
    ...
    let (runtime, worker) = {
        let worker_ref = worker.take_inner()?;
        (worker_ref.core_runtime, worker_ref.core_worker)
    };

    let worker = Arc::try_unwrap(worker).map_err(|arc| BridgeError::IllegalStateStillInUse {
        what: "Worker",
        details: Some(format!(
...
pub fn push_history(
    ...
    let pusher_ref = pusher.borrow_inner()?;
...
pub fn close_history_stream(
    ...
    let _pusher_ref = pusher.take_inner()?;
    Ok(())
}
    ...
    impl<SK: SlotKind + Send + Sync + 'static> CoreSlotSupplier for SlotSupplierBridge<SK> {
        ...
        async fn reserve_slot(
            ...
            loop {
                ...
                let (_abort_controller, abort_signal) =
                    AbortController::new("Request Cancelled".to_string());
