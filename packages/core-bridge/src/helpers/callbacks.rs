...
///
/// Note that at this level, a callback is a pure function, not a method on an object;
/// that is, the callback will be called with `this` set to `undefined`. If the JS side
/// API presents the callback as a method on some object, then the function should be
/// `bind(this)`'d on the JS side before passing it to the Rust side.
        ...
        Self {
            inner: Arc::new(CallbackInner {
                ...
                func_name: func
                    .to_string(cx)
                    .map(|s| s.value(cx))
                    .unwrap_or("anonymous func".to_owned()),
                chan: cx.channel(),
                _marker: PhantomData,
            }),
        }
    ...
    Ret: TryFromJs + Send + Sync + 'static,
{
    fn try_from_js<'cx, 'b>(
        ...
        Ok(JsCallback::<Args, Ret>::new(cx, func, None))
    }
}
    ...
    Ret: TryFromJs + Send + Sync + 'static,
{
    ...
    pub async fn call(&self, args: Args) -> BridgeResult<Ret> {
        ...
        let fut = res.map_err(|e| BridgeError::Other(e.into()))?;

        // ... and then wait for the Promise to resolve
        let ret = fut.await;
        let ret = ret.map_err(|e| BridgeError::Other(e.into()))??;
        Ok(ret)
    }
}
    ...
    Ret: TryFromJs + Send + Sync + 'static,
{
    fn try_from_js<'cx, 'b>(
        ...
        let func = js_value.downcast::<JsFunction, _>(cx)?;
        Ok(JsAsyncCallback::<Args, Ret>::new(cx, func, None))
    }
}
...
impl<R: TryFromJs + Send + Sync + 'static> TryFromJs for JsFuture<BridgeResult<R>> {
    fn try_from_js<'cx, 'b>(
        ...
        let prom = js_value.downcast::<JsPromise, _>(cx)?;
        let fut = prom.to_future(cx, |mut cx, result| match result {
            // to_future() expects a function that throws a JS ,
            Ok(value) => {
...
impl<Args: TryIntoJsArgs + Send + Sync, Ret: TryFromJs + Send + Sync> CallbackInner<Args, Ret> {
    pub fn call<'a, C: Context<'a>>(&self, cx: &mut C, args: Args) -> BridgeResult<Ret> {
        let this: Handle<'a, JsValue> = self
            .this
            .as_ref()
            .map(|t| t.to_inner(cx).upcast())
