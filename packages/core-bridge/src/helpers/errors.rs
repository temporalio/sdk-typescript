...
pub enum BridgeError {
    ...
    #[error("{}{message}", field_prefix(.field))]
    TypeError {
        field: Option<String>,
        message: String,
    },
    ...
    #[error("{what} still in use{}", details_suffix(.details))]
...
impl BridgeError {
    fn into_throw<'cx, C: Context<'cx>, T>(self, cx: &mut C) -> NeonResult<T> {
        match self {
            Self::TypeError { .. } => cx.throw_type_error(self.to_string()),
            Self::InvalidVariant { .. } => cx.throw_type_error(self.to_string()),

            Self::TransportError(..) => throw_custom_error(cx, TRANSPORT_ERROR, self.to_string()),

            Self::IllegalStateStillInUse { .. } => {
                throw_custom_error(cx, ILLEGAL_STATE_ERROR, self.to_string())
            }

            Self::IllegalStateAlreadyClosed { .. } => {
                throw_custom_error(cx, ILLEGAL_STATE_ERROR, self.to_string())
            }
