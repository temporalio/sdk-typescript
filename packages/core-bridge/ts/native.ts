/**
 * Indicates a property that is allowed to be unspecified when calling in or out
 * of native code (the equivalent of the `Option<T>` type in Rust).
 *
 * Always use either this type or the `T | null` idiom to indicate a property
 * that may legitimately be left unspecified when calling into or out of native
 * code. Never use `T | undefined` or `prop?: T` on TS/Rust interfaces.
 *
 * ### Rationale
 *
 ...
 *
 * Thanks to those conventions, a property that reads as `undefined` is known to
 * to always indicate an _unintentionally missing_ property, which will results
 * in a runtime error.
 */
...
export declare function newRuntime(telemOptions: CoreRuntimeOptions): Runtime;
