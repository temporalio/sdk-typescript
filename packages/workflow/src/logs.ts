...
export function executeWithLifecycleLogging(fn: () => Promise<unknown>): Promise<unknown> {
  log.debug('Workflow started', { logSource: LogSource.workflowWorker });
  const p = fn().then(
    (res) => {
      log.debug('Workflow completed', { logSource: LogSource.workflowWorker });
      return res;
    },
    (error) => {
      ...
      if (typeof error === 'object' && error != null) {
        if (isCancellation(error)) {
          log.debug('Workflow completed as cancelled', { logSource: LogSource.workflowWorker });
          throw error;
        } else if (error instanceof ContinueAsNew) {
          log.debug('Workflow continued as new', { logSource: LogSource.workflowWorker });
          throw error;
        }
      }
      log.warn('Workflow failed', { error, logSource: LogSource.workflowWorker });
      throw error;
    }
  );
