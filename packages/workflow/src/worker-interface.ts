...
import { encodeVersioningBehavior, IllegalStateError, isWorkflowFunctionWithOptions, isWorkflowFunctionWithStaticOptions } from '@temporalio/common';
...
export function initRuntime(options: WorkflowCreateOptionsInternal): void {
  ...
  try {
    ...
    if (isWorkflowFunctionWithStaticOptions(activator.workflow)) {
      activator.typeHints = activator.workflow.staticOptions.typeHints;
    }
  } catch (e) {
    try {
