...
/**
 * Updates the activator's state in preparation for a activation. This is equivalent to the special
 * bookkeeping work that was previously done in the `activate` function on batchIndex 0.
 *
 * This function also handles incoming `notifyPatches` jobs and update `sdkFlags`, except for patches
 * and SDK flags that are defined on the very first WFT, as those need to be set before we load
 * workflow bundle code (see {@link initRuntime}).
 */
export function startActivation(activation: coresdk.workflow_activation.IWorkflowActivation): void {
  const activator = getActivator();
  // FIXME: I'm keeping the internal interceptor contract as-is for now, but we need to revisit at some point in the future
  const intercept = composeInterceptors(activator.interceptors.internals, 'activate', ({ activation }) => {
    ...
    activator.addKnownPatches(
      (activation.jobs?.map((job) => job.notifyHasPatch) ?? []) as coresdk.workflow_activation.INotifyHasPatch[]
    );
  });
