...
/**
 * Options used to create a Temporal Runtime. These are mostly about logging and telemetry.
 */
...
export type LogExporterConfig = {
   ...
   *
   *
   * Note that if log forwarding is enabled, then the configured {@link Runtime.logger} may apply
   * further filtering on top of this.
   *
   *
   ...
   *
   * @default `{ core: 'WARN', other: 'ERROR'}` (with some exceptions, as described in backward compatibility note above).
   */
...
export function compileOptions(options: RuntimeOptions): CompiledRuntimeOptions {
  ...
  const { metrics, noTemporalPrefixForMetrics } = options.telemetryOptions ?? {};
...
function compileLoggerOptions(options: RuntimeOptions): [Logger, native.LogExporterOptions] {
  // eslint-disable-next-line deprecation/deprecation
  const { logging, tracingFilter } = options.telemetryOptions ?? {};
