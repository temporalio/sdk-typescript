...
export class Worker {
    ...
    const filteredCalls = externalCalls
      ...
      .filter(({ call: { ifaceName, fnName }, sink }) => {
        ...
        this.log.error('Workflow referenced an unregistered external sink', {
          ifaceName,
          fnName,
          logSource: LogSource.workflowWorker,
        });
        return false;
      })
  ...
  protected activityHeartbeat$(): Observable<void> {
    ...
    return this.activityHeartbeatSubject.pipe(
      ...
      tap({
        complete: () => this.log.debug('Heartbeats complete', { logSource: LogSource.activityWorker }),
      }),
      closeableGroupBy(({ base64TaskToken }) => base64TaskToken),
      mergeMap((group$) =>
        group$.pipe(
  ...
  protected workflowPoll$(): Observable<WorkflowActivation> {
    return this.pollLoop$(async () => {
      ...
      this.log.trace('Got workflow activation', { ...activation, logSource: LogSource.workflowWorker });
  ...
  protected workflow$(): Observable<void> {
    ...
    if (this.workflowCreator === undefined) {
      this.log.warn('No workflows registered, not polling for workflow tasks', { logSource: LogSource.workflowWorker });
      this.workflowPollerStateSubject.next('SHUTDOWN');
      return EMPTY;
    }
    return this.workflowPoll$().pipe(
      ...
      tap({
        complete: () => {
          this.log.debug('Workflows complete', { logSource: LogSource.workflowWorker });
        },
      })
    );
  }
  ...
  protected activity$(): Observable<void> {
    ...
    return this.activityPoll$().pipe(
      ...
      tap({ complete: () => this.log.debug('Activities complete', { logSource: LogSource.activityWorker }) })
    );
  }
