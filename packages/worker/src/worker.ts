...
export class NativeWorker implements NativeWorkerLike {
  ...
  public static async createReplay(options: CompiledWorkerOptionsWithBuildId): Promise<NativeReplayHandle> {
    const runtime = Runtime.instance();
    const replayer = await runtime.createReplayWorker(toNativeWorkerOptions(options));
    return {
...
export class Worker {
   ...
   * confirmed that it's shutting down, the Worker enters `'DRAINING'` state. It will stay in that state until
   * both task pollers receive a `ShutdownError`, at which point we'll transition to `DRAINED` state. Once all
   * currently running Activities and Workflow Tasks have completed, the Worker transitions to `'STOPPED'`.
   */
  shutdown(): void {
