...
export interface MetricHistogram extends Metric {
   ...
   *
   * @param value Value to record. Must be a non-negative number. Value will be
   *              casted to the given {@link valueType}. Loss of precision may
   *              occur if the value is not already of the correct type.
   * @param extraTags Extra tags if any.
   */
...
export class MetricsMeterWithComposedTags implements MetricMeter {
   ...
   *                   evaluated every time a metric is recorded that will return a delta object.
   * @param force if `true`, then a `MetricsMeterWithComposedTags` will be created even if
   *              there is no tags to add. This is useful to add tags support to an underlying
   *              meter implementation that does not support tags directly.
   */
