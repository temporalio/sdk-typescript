    ...
    srv.listen({ port: 0, host: '127.0.0.1' }, () => {
      ...
      fn(addr.port)
        ...
        .finally(() => {
          ...
          // Delaying closing the server by 1 second is enough to avoid that situation.
          setTimeout(() => {
            srv.close();
          }, 1000).unref();
        });
    ...
    srv.listen({ port: 0, host: '127.0.0.1' }, () => {
      ...
      fn(addr.port)
        ...
        .finally(() => {
          ...
          // The OTel exporter will try to flush metrics on drop, which may result in ERROR messages
          // on the console if the server has had time to complete shutdown before then.
          // Not really a problem, but delaying closing the server by 1 second helps avoid the error.
          setTimeout(() => {
            srv.close();
          }, 1000).unref();
        });
