...
if (RUN_INTEGRATION_TESTS) {
  ...
  test.serial('Runtime.install() remembers installed options after it has been shut down', async (t) => {
    ...
    t.is(Runtime.instance().options.logger, logger);
  ...
  test.serial('Runtime.install() Core forwarded logs contains metadata', async (t) => {
    ...
    try {
      ...
      await worker.runUntil(() =>
        asyncRetry(
          () => {
            if (!logEntries.some((entry) => entry.message === 'Failing workflow task')) {
      ...
      const initWorkerEntry = logEntries.find((entry) => entry.message === 'Initializing worker');
      t.truthy(initWorkerEntry);
      t.is(initWorkerEntry?.meta?.['taskQueue'], 'q1');

      const failingWftEntry = logEntries.find((entry) => entry.message === 'Failing workflow task');
      t.truthy(failingWftEntry);
  ...
  test.serial(`NativeLogCollector: Buffered logs are periodically flushed even if Core isn't flushing`, async (t) => {
    ...
    try {
      ...
      t.true(logEntries.some((entry) => entry.message.startsWith('workflow log ')));

      bufferedLogger.info('final log');
      t.false(logEntries.some((entry) => entry.message.startsWith('final log')));
    } finally {
      await connection.close();
      await runtime.shutdown();
    }

    t.is(logEntries.filter((entry) => entry.message.startsWith('workflow log ')).length, 5);
    t.is(logEntries.filter((entry) => entry.message.startsWith('final log')).length, 1);
  });
}
