...
test('Worker can replace connection to switch servers', async (t) => {
  ...
  const { createWorker, createNativeConnection, startWorkflow, taskQueue } = helpers(t);
  ...
  const handle1 = await env1.client.workflow.start(tickingWorkflow, { taskQueue, workflowId: randomUUID(), workflowExecutionTimeout: '300s' });
  
  const env2 = await TestWorkflowEnvironment.createLocal({});
  const handle2 = await env2.client.workflow.start(tickingWorkflow, { taskQueue, workflowId: randomUUID(), workflowExecutionTimeout: '300s' });
