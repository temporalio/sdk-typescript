...
import { TypedSearchAttributes, SearchAttributes, TypedSearchAttributePair, SearchAttributeType, searchAttributePair, encodeSearchAttributeIndexedValueType, TypedSearchAttributeUpdatePair, searchAttributeUpdatePair } from '@temporalio/common';
import { temporal } from '@temporalio/proto';
import { waitUntil } from './helpers';
import { condition, defineQuery, defineSignal, setHandler, upsertSearchAttributes, WorkflowInfo, workflowInfo } from '@temporalio/workflow';
...
if (test?.serial?.before) {
  ...
  test.serial.before(async (t) => {
    ...
    const untypedKeys = Object.entries(untypedAttrsInput).reduce((acc, [key, value]) => {
    ...
    const typedKeys = typedAttrsListInput.reduce((acc, [key]) => {
    ...
    t.context.env.connection.operatorService.addSearchAttributes({
    ...
    await waitUntil(
      async () => {
        const resp = await t.context.env.connection.operatorService.listSearchAttributes({ namespace: t.context.env.namespace });
        return Object.keys(untypedKeys).every(key => key in resp.customAttributes) && Object.keys(typedKeys).every(key => key in resp.customAttributes);
      },
      300,
    );
  });
...
test('creating schedules with various input search attributes', async (t) => {
  await Promise.all(inputTestCases.map(async ({ input, expected: output }) => {
    const { taskQueue } = helpers(t);
    const client = t.context.env.client;
    const action: ScheduleOptionsAction = {
      type: 'startWorkflow',
...
export const mutateSearchAttributes = defineSignal<[SearchAttributes | TypedSearchAttributeUpdatePair[]]>('mutateSearchAttributes');
...
test('upsert works with various search attribute mutations', async (t) => {  
  ...
  await worker.runUntil(async () => {
    ...
    let { typed_bool, ...descExpected } = untypedUpdateAttrs;

    assertWorkflowInfoSearchAttributes(t, res, untypedUpdateAttrs, new TypedSearchAttributes([
    ...
    assertWorkflowDescSearchAttributes(t, desc, descExpected, new TypedSearchAttributes(
      [
        searchAttributePair("typed_text", SearchAttributeType.TEXT, "new_value"),
    ...
    const { typed_keyword, typed_datetime, ...remaining } = expectedUntyped;
    ...
    assertWorkflowDescSearchAttributes(t, desc, remaining, expectedTyped);

    await handle.signal(complete);
  });
});

function assertWorkflowInfoSearchAttributes(t: ExecutionContext<Context>, res: WorkflowInfoWithMeta, searchAttributes: SearchAttributes, typedSearchAttributes: TypedSearchAttributes) {
...
function assertWorkflowDescSearchAttributes(t: ExecutionContext<Context>, desc: WorkflowExecutionDescription, searchAttributes: SearchAttributes, typedSearchAttributes: TypedSearchAttributes) {
  ...
  let descOmittingBuildIds = desc.typedSearchAttributes.updateSearchAttributes([searchAttributeUpdatePair("BuildIds", SearchAttributeType.BOOL, null)])
