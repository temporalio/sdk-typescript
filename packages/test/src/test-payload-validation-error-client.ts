...
test('client interceptor headers fail codec encoding locally without RPCs', async (t) => {
  const failure = createPayloadValidationError({ field: 'header' });
  let rpcCount = 0;
  const workflowService = new Proxy(
    {},
    {
  ...
  const invalidHeader = defaultPayloadConverter.toPayload({ rejectHeader: true });
  const headers = { payloadValidation: invalidHeader };
  const client = new Client({
      ...
      payloadCodecs: [
        {
          async encode(payloads) {
            if (payloads.some((payload) => defaultPayloadConverter.fromPayload<any>(payload)?.rejectHeader)) {
              throw new Error('codec wrapper', { cause: failure });
            }
            return payloads;
          },
    ...
    interceptors: {
      workflow: [
        {
          startWithDetails: (input, next) => next({ ...input, headers }),
          signal: (input, next) => next({ ...input, headers }),
          signalWithStart: (input, next) => next({ ...input, headers }),
          query: (input, next) => next({ ...input, headers }),
          startUpdate: (input, next) => next({ ...input, headers }),
        },
      ],
  ...
  const calls: Array<() => Promise<unknown>> = [
    () => client.workflow.start('workflow', { workflowId: 'workflow-id', taskQueue: 'queue', args: ['safe'] }),
    () => handle.signal('signal', 'safe'),
    () =>
      client.workflow.signalWithStart('workflow', {
        workflowId: 'workflow-id',
        taskQueue: 'queue',
    ...
    () => handle.query('query', 'safe'),
    () => handle.executeUpdate('update', { args: ['safe'] }),
    () =>
      client.activity.start('activity', {
        id: 'activity-id',
        taskQueue: 'queue',
