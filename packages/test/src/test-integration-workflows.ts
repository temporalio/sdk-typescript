...
export async function runTestActivity(
  activityOptions?: workflow.ActivityOptions
): Promise<void> {
  await workflow
    .proxyActivities({ startToCloseTimeout: "1m", ...activityOptions })
    .testActivity();
}
...
export async function cancelFakeProgress(): Promise<void> {
  ...
  await workflow.CancellationScope.cancellable(async () => {
    ...
    await new Promise<void>((resolve) =>
      workflow.setHandler(activityStartedSignal, resolve)
    );
...
test("HistorySize grows with new WFT", async (t) => {
  const { createWorker, executeWorkflow } = helpers(t);
  const worker = await createWorker();
  const [before, after] = await worker.runUntil(
    executeWorkflow(historySizeGrows)
  );
...
export async function suggestedCAN(): Promise<boolean> {
  ...
  while (workflow.workflowInfo().historyLength < maxEvents) {
    await Promise.all(
      new Array(batchSize).fill(undefined).map((_) => workflow.sleep(1))
    );
    if (workflow.workflowInfo().continueAsNewSuggested) {
      return true;
    }
...
test("Activity initialInterval is not getting rounded", async (t) => {
  ...
  const activityTaskScheduledEvents = events?.find(
    (ev) => ev.activityTaskScheduledEventAttributes
  );
  const retryPolicy =
    activityTaskScheduledEvents?.activityTaskScheduledEventAttributes
      ?.retryPolicy;
...
test("Start of workflow is delayed", async (t) => {
  ...
  const { events } = await handle.fetchHistory();
  const workflowExecutionStartedEvent = events?.find(
    (ev) => ev.workflowExecutionStartedEventAttributes
  );
  const startDelay =
    workflowExecutionStartedEvent?.workflowExecutionStartedEventAttributes
...
test("Start of workflow with signal is delayed", async (t) => {
  ...
  // This workflow never runs
  const handle = await t.context.env.client.workflow.signalWithStart(
    workflows.interruptableWorkflow,
    {
      ...
      .then((res) => {
        if (res !== "workflow-and-activity-worker")
          throw workflow.ApplicationFailure.nonRetryable(
            "Activity was not eagerly dispatched"
          );
      });
...
test("Worker requests Eager Activity Dispatch if possible", async (t) => {
  ...
  const activityTaskStarted = events?.filter?.(
    (ev) => ev.activityTaskStartedEventAttributes
  );
  t.is(activityTaskStarted?.length, 30);
  t.true(
    activityTaskStarted?.every(
      (ev) => ev.activityTaskStartedEventAttributes?.attempt === 1
    )
  );
});
...
test("Worker doesn't request Eager Activity Dispatch if no activities are registered", async (t) => {
  ...
  const result = await activityWorker.runUntil(
    workflowWorker.runUntil(handle.result())
  );
  const { events } = await handle.fetchHistory();
  ...
  t.is(
    activityTaskStarted?.[0]?.activityTaskStartedEventAttributes?.attempt,
    1
  );
});
