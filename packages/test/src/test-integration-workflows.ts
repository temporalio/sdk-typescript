...
import { CountWorkflowExecution, isGrpcServiceError, ServiceError, WorkflowFailedError } from '@temporalio/client';
...
test('can register search attributes to dev server', async (t) => {
  const { startWorkflow } = helpers(t);
  const key = TypedSearchAttributes.createKey('new-search-attr', SearchAttributeType.INT);
  const newSearchAttribute = TypedSearchAttributes.createAttribute(key.name, key.type, 12);
