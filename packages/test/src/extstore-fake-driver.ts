...
export interface FakeDriverOptions {
  ...
  name?: string;
  /** Override default in-memory store. Receives the context so a fake can take permits. */
  onStore?: (
    payloads: Payload[],
    context: StorageDriverStoreContext
  ) => StorageDriverClaim[] | Promise<StorageDriverClaim[]>;
  /** Override default in-memory retrieve. Receives the context so a fake can take permits. */
  onRetrieve?: (claims: StorageDriverClaim[], context: StorageDriverRetrieveContext) => Payload[] | Promise<Payload[]>;
...
export function makeFakeDriver(opts: FakeDriverOptions = {}): FakeDriver {
  ...
  const driver: FakeDriver = {
    ...
    async store(context, payloads) {
      ...
      if (opts.onStore) return opts.onStore(payloads, context);
    ...
    async retrieve(context, claims) {
      ...
      if (opts.onRetrieve) return opts.onRetrieve(claims, context);
