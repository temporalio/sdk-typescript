...
import { Client, WorkflowHandle, WorkflowHandleWithFirstExecutionRunId } from '@temporalio/client';
...
test('ContinueAsNew with version upgrade', async (t) => {
  ...
  const wfHandle = await client.workflow.start('continueAsNewWithVersionUpgrade', {
    taskQueue,
    workflowId: 'can-upgrade-' + randomUUID(),
    args: [0],
  });

  // Wait for workflow to be running on v1
  await waitForWorkflowRunningOnVersion(client, wfHandle, v1.buildId);
