...
import { Context as BaseContext, makeTestFunction } from './helpers-integration';
...
test.serial('Buffered Metrics - Exporting buffered metrics from Core works properly', async (t) => {
  ...
  const gaugeFloat = meter.createGauge('my-float-gauge', 'float', 'my-float-gauge-unit', 'my-float-gauge-description');
  gaugeFloat.set(1.1);
  gaugeFloat.set(1.1);
  gaugeFloat.set(40.1);
  ...
  const updatesIterator = Runtime.instance().retrieveBufferedMetrics();
