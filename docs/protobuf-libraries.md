...
## Alternative: `@bufbuild/protobuf` v2

The protobufjs path remains the default for compatibility. For users that prefer Buf's modern toolchain (`@bufbuild/protobuf` v2 with code generated by `@bufbuild/protoc-gen-es`) we ship a parallel set of payload converters exported from `@temporalio/common/lib/protobufs-es`:

- `ProtobufEsBinaryPayloadConverter`
- `ProtobufEsJsonPayloadConverter`
- `DefaultPayloadConverterWithProtobufsEs` — composite, ordering matches `DefaultPayloadConverterWithProtobufs` (Undefined → Binary → ProtobufJson → ProtobufBinary → Json)

Both protobuf-es converters are wire-compatible with their protobufjs counterparts: the on-the-wire `encoding` metadata, fully-qualified `messageType` metadata, and serialized binary bytes are identical for the same logical message. A worker on one runtime can decode a payload produced by a client on the other in either binary or JSON form.

JSON is semantically equivalent, not byte-identical. Both converters emit conforming proto3 JSON, but their defaults differ:

- `ProtobufEsJsonPayloadConverter` matches `@bufbuild/protobuf`'s default, which matches the Go SDK's `protojson.Marshal` default: fields with implicit (zero) values are omitted. `create(Schema, { name: '', age: 0 })` round-trips as `{}`.
- `ProtobufJsonPayloadConverter` (protobufjs + `proto3-json-serializer`) emits implicit defaults. The same message round-trips as `{"name":"","age":0}`.

Both forms parse correctly by either reader, and the in-memory representation after decode is identical, so this is a wire-bytes difference only — not a correctness gap. If exact byte parity matters for your stack, subclass `ProtobufEsJsonPayloadConverter` and override `toPayload` to pass `{ alwaysEmitImplicit: true }` to `toJson`, or normalize on the protobufjs side.

`@bufbuild/protobuf` is declared as an optional peer dependency on `@temporalio/common`, so users on the protobufjs path don't pay for it.

```ts
// src/payload-converter.ts
import { createRegistry } from '@bufbuild/protobuf';
import { DefaultPayloadConverterWithProtobufsEs } from '@temporalio/common/lib/protobufs-es';
import { ProtoActivityInputSchema, ProtoActivityResultSchema } from '../gen/messages_pb';

const registry = createRegistry(ProtoActivityInputSchema, ProtoActivityResultSchema);
export const payloadConverter = new DefaultPayloadConverterWithProtobufsEs({ registry });

// src/worker.ts and src/client.ts (unchanged from the protobufjs flow):
//   dataConverter: { payloadConverterPath: require.resolve('./payload-converter') }

// src/workflows.ts
import { create } from '@bufbuild/protobuf';
import { ProtoActivityResultSchema, type ProtoActivityInput } from '../gen/messages_pb';

export async function protoWorkflow(input: ProtoActivityInput) {
  return create(ProtoActivityResultSchema, { sentence: `Name is ${input.name}` });
}
```

Key differences from the protobufjs flow:

- Use `create(MessageSchema, init)` instead of `Message.create(init)`. There is no `new Message()` form either.
- Construct a `Registry` (or pass a schema array, which the converters will pass to `createRegistry` internally) instead of a `protobufjs` `Root`.
- No `patchProtobufRoot` step — `protoc-gen-es` already generates types that work directly with TypeScript.
- Spec-compliant proto3 JSON is built in (no `proto3-json-serializer` needed).
