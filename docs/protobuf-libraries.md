...
For code generated by `@bufbuild/protoc-gen-es`, a parallel set of converters is exported from `@temporalio/common/lib/protobufs-es`: `ProtobufEsBinaryPayloadConverter`, `ProtobufEsJsonPayloadConverter`, and `DefaultPayloadConverterWithProtobufsEs`. Binary payloads are byte-identical to the protobufjs converters. JSON payloads use canonical proto3 JSON, omit implicit (zero) fields by default, and follow `@bufbuild/protobuf` semantics for numeric types like `int64`, matching the Go SDK's `protojson.Marshal`.

`@bufbuild/protobuf` is an optional peer dependency.
