...
export type RequiredWorkflowOptions= Required<Pick<BaseWorkflowOptions, 'workflowId' | 'workflowIdReusePolicy' | 'taskQueue'>>;

export type WorkflowOptionsWithDefaults = WorkflowOptions & RequiredWorkflowOptions;

export type CompiledWorkflowOptionsWithDefaults = BaseWorkflowOptions & RequiredWorkflowOptions & {
...
export class Connection {
  ...
  public async untilComplete(workflowId: string, runId: string) {
    ...
    if (ev.workflowExecutionCompletedEventAttributes) {
      ...
      const [result] = arrayFromPayloads(this.options.dataConverter, ev.workflowExecutionCompletedEventAttributes.result);
      return result;
    } else if (ev.workflowExecutionFailedEventAttributes) {
      throw new Error(ev.workflowExecutionFailedEventAttributes.failure?.message || 'Workflow failed without failure message');
